...
for rev in all_revisions:
    ...
    rev_table += rev_table_template.format(number=str(rev.SequenceNumber),
...
class RevisedSheet:
    ...
    def _find_all_revisions(self):
        ...
        # 17-05-2019: Added try/except to catch revision number errors and replace with sequence
        try: 
            self._rev_numbers = set([revit.doc.GetElement(rev_id).RevisionNumber
                                     for rev_id in self._revisions])
        except:
            self._rev_numbers = set([str(revit.doc.GetElement(rev_id).SequenceNumber)
                                     for rev_id in self._revisions])
